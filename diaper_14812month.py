...
c.execute('SELECT k2.kidID, k2.activityHour, 1.0*count(k2.activity) / month.total \
            ...
            LEFT JOIN \
             ...
             ON k2.kidID = month.id \
                AND k2.activityAgeMonths = month.age \
             WHERE k2.activityAgeMonths == 3 AND k2.activity == '"'Diaper'"' \
             GROUP BY k2.kidID, k2.activityHour')
data = c.fetchall() # output is list of tuples
...
c.execute('SELECT k2.kidID, k2.activityHour, 1.0*count(k2.activity) / month.total \
            ...
            LEFT JOIN \
             ...
             ON k2.kidID = month.id \
                AND k2.activityAgeMonths = month.age \
             WHERE k2.activityAgeMonths == 7 AND k2.activity == '"'Diaper'"' \
             GROUP BY k2.kidID, k2.activityHour')
data = c.fetchall() # output is list of tuples
...
c.execute('SELECT k2.kidID, k2.activityHour, 1.0*count(k2.activity) / month.total \
            ...
            LEFT JOIN \
             ...
             ON k2.kidID = month.id \
                AND k2.activityAgeMonths = month.age \
             WHERE k2.activityAgeMonths == 11 AND k2.activity == '"'Diaper'"' \
             GROUP BY k2.kidID, k2.activityHour')
data = c.fetchall() # output is list of tuples
...
# output to tsv
dfOutput.to_csv(outFile + 'diaper_14812month.tsv', sep='\t', na_rep='NaN')
