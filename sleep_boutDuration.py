...
# DAY
c.execute('SELECT k.kidID, k.activityAgeMonths, 1.0*AVG(k.durationMin) \
            FROM Kids AS k \
...
dfmeta = pd.DataFrame(dfData2.median(axis=1), columns={'sleepDurDay'})
dfmeta['sleepDurDaySTD1'] = dfmeta['sleepDurDay'] + dfData2.std(axis=1) 
dfmeta['sleepDurDaySTD2'] = dfmeta['sleepDurDay'] - dfData2.std(axis=1)
...
# calculate the meta information
dfmeta = pd.DataFrame(dfData2.median(axis=1), columns={'sleepDurNight'})
dfmeta['sleepDurNightSTD1'] = dfmeta['sleepDurNight'] + dfData2.std(axis=1) 
dfmeta['sleepDurNightSTD2'] = dfmeta['sleepDurNight'] - dfData2.std(axis=1)
...
# output to tsv
dfOutput.to_csv(outFile + 'sleep_boutDuration.tsv', sep='\t', na_rep='NaN')
